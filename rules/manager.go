...
// EngineQueryFunc返回一个新的query函数，对给定的engine执行即时查询
	...
	return func(ctx context.Context, qs string, t time.Time) (promql.Vector, error) {
		...
		case promql.Scalar:
			return promql.Vector{promql.Sample{
				// 纯量的话，就直接手动构建一个Sample出来
				Point:  promql.Point(v),
				Metric: labels.Labels{},
			}}, nil
		default:
			// rule的查询结果必须是vector或者scalar
...
// CopyState从给定的group中拷贝alerting rule以及staleness related state
//
...
// NotifyFunc发送给定的expression产生的一系列alerts相关的notifications
type NotifyFunc func(ctx context.Context, expr string, alerts ...*Alert)

// ManagerOptions bundles options for the Manager.
// ManagerOptions打包对于Manager的配置
...
func (m *Manager) LoadGroups(interval time.Duration, filenames ...string) (map[string]*Group, []error) {
	...
	for _, fn := range filenames {
		...
		for _, rg := range rgs.Groups {
			...
			// group的名称加上rule file的名字合起来作为key
